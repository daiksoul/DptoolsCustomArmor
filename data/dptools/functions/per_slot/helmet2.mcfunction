...
#Move item to entity.
#Get values.
execute store result score $TEMP_DURABILITY carm.val run data get entity @s Inventory[{Slot:103b}].tag.cdur.current_d
execute store result score $TEMP_DURABILITY2 carm.val run data get entity @s Inventory[{Slot:103b}].tag.cdur.max_d
...
execute if score $TEMP_DURABILITY carm.val matches ..0 run item replace entity @s armor.head with air
execute if score $TEMP_DURABILITY carm.val matches ..0 at @s run playsound minecraft:entity.item.break master @a[distance=..4]
#Create the lore.
data modify storage dptools:cdur TMPLORESTR set from entity @s Inventory[{Slot:103b}].tag.display.Lore
...
item modify entity @s armor.head dptools:copylore
item modify entity @s armor.head dptools:setlore