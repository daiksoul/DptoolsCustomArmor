...
execute if data entity @s Item.tag.cdur.repair_item.id run tag @s add carm.idReq
execute if data entity @s Item.tag.cdur.repair_item.tag run tag @s add carm.tagReq

#Compare item id with repair_item tag
execute if entity @s[tag=carm.idReq] run data modify entity @s Item.tag.cdur.repair_item2.id set from entity @s Item.tag.cdur.repair_item.id
execute if entity @s[tag=carm.idReq] if entity @e[type=item,sort=furthest,tag=!carm.checked,distance=..1] store success score $TMP_REPAIR carm.val run data modify entity @s Item.tag.cdur.repair_item2.id set from entity @e[type=item,sort=furthest,tag=!carm.checked,limit=1,distance=..1] Item.id

execute if entity @s[tag=carm.tagReq] run data modify entity @s Item.tag.cdur.repair_item2.tag set from entity @s Item.tag.cdur.repair_item.tag
execute if entity @s[tag=carm.tagReq] if entity @e[type=item,sort=furthest,tag=!carm.checked,distance=..1] if data entity @e[type=item,sort=furthest,tag=!carm.checked,distance=..1,limit=1] Item.tag.cdur.cmat store success score $TMP_REPAIR2 carm.val run data modify entity @s Item.tag.cdur.repair_item2.tag set from entity @e[type=item,sort=furthest,tag=!carm.checked,limit=1,distance=..1] Item.tag.cdur.cmat
...
execute if entity @s[tag=carm.repa1] run function dptools:repair/exp_check
...
execute if entity @e[tag=!carm.repa1] if entity @e[type=item,tag=!carm.checked,distance=..1] run function dptools:repair/mat_check

scoreboard players reset $TMP_REPAIR carm.val
scoreboard players reset $TMP_REPAIR2 carm.val
data remove entity @s Item.tag.cdur.repair_item2
