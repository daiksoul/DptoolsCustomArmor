...
#Move item to entity.
#Get values.
execute store result score $TEMP_DURABILITY carm.val run data get entity @s Item.tag.cdur.current_d
execute store result score $TEMP_DURABILITY2 carm.val run data get entity @s Item.tag.cdur.max_d
execute store result score $TEMP_DURABILITY3 carm.val run data get entity @s Item.tag.cdur.max_d
...
data modify storage dptools:cdur TMPLORESTR set from entity @e[tag=carm.temp,limit=1,sort=nearest] HandItems[0].tag.display.Lore
