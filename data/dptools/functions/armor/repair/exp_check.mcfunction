...
data modify storage dptools:armor PLAYERTMP set from entity @s Thrower
execute store success score $PLAYERCHECK carm.val run data modify storage dptools:armor PLAYERTMP set from entity @p[tag=!carm.checked] UUID
...
execute if entity @s[tag=carm.exp_s] if entity @p[tag=carm.repair_own,level=1..] run function dptools:armor/repair/repair
...
execute if entity @s[tag=!carm.exp_s] if entity @p[tag=!carm.checked] run function dptools:armor/repair/exp_check

scoreboard players reset $PLAYERCHECK carm.val
tag @s remove carm.exp_s
tag @a[tag=carm.checked] remove carm.checked
data remove storage dptools:armor PLAYERTMP