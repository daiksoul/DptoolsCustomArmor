...
#Move item to entity.
#Get values.
execute store result score $TEMP_DURABILITY carm.val run data get entity @s Inventory[{Slot:103b}].tag.current_d
execute store result score $TEMP_DURABILITY2 carm.val run data get entity @s Inventory[{Slot:103b}].tag.max_d
...
#Create the lore.
data modify storage dptools:armor TMPLORESTR set from entity @s Inventory[{Slot:103b}].tag.display.Lore
